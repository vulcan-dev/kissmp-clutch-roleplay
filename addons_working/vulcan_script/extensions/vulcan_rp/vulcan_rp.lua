...
M.callbacks = {
    ...
    VK_Tick = function()
        ...
        for _, client in pairs(G_Clients) do
            if client.user:getID() ~= 1337 then
                --[[ Dragging ]]--
                if client.connected and client.cuffed.isBeingDragged then
                ...
                if client.gps.enabled then
                    ...
                    if my_vehicle then
                        ...
                        if (x > client.gps.position.x - 8 and x < client.gps.position.x + 8) and (y > client.gps.position.y - 8 and y < client.gps.position.y + 8) then
