...
local function InitializeMarkers(client)
    ...
    for townName, fuelStation in pairs(modules.utilities.GetKey(G_Locations, 'fuel_pumps')) do
        for _, location in pairs(fuelStation) do
            count = count + 1
            CreateMarker(client, 'pump_'..townName..count, 'art/shapes/interface/checkpoint_marker_base.dae', location.x, location.y, location.z)
        end
    end
    ...
    for storeName, store in pairs(modules.utilities.GetKey(G_Locations, 'robbable_shops')) do
        count = count + 1
        CreateMarker(client, 'repair_'..storeName..count, 'art/shapes/interface/checkpoint_marker_base.dae', store.x, store.y, store.z, {0.7, 0.19, 0.23, 1}, {1, 1, 1})
    end
    ...
    for mechanicName, repairStation in pairs(modules.utilities.GetKey(G_Locations, 'repair_stations')) do
        ...
        if type(repairStation.x) == 'number' and type(repairStation.y) == 'number' and type(repairStation.z) == 'number' then
            CreateMarker(client, 'repair_'..mechanicName..count, 'art/shapes/interface/checkpoint_marker_base.dae', repairStation.x, repairStation.y, repairStation.z, {0.8, 0.4, 0.03, 1}, {4, 4, 1})
        else
            for _, location in pairs(repairStation) do
                CreateMarker(client, 'repair_'..mechanicName..count, 'art/shapes/interface/checkpoint_marker_base.dae', location.x, location.y, location.z, {0.8, 0.4, 0.03, 1}, {2, 2, 1})
            end
        end
    end
